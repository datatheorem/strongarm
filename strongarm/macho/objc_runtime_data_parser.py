...
class ObjcSelector:
    ...
    def __init__(
        self,
        name: str,
        selref: Optional["ObjcSelref"],
        implementation: Optional[VirtualMemoryPointer],
    ) -> None:
        self.name = name
        self.selref = selref
...
class ObjcIvar:
    ...
    def __init__(
        ...
        class_name: str,
        offset: int,
        field_offset_addr: VirtualMemoryPointer,
    ):
        self.name = name
...
class ObjcRuntimeDataParser:
    def __init__(self, binary: MachoBinary) -> None:
        ...
        self._selector_literal_ptr_to_selref_map: Dict[
            VirtualMemoryPointer, ObjcSelref
        ] = {}
    ...
    def _parse_linked_dylib_symbols(self) -> Dict[str, str]:
        ...
        for undef_sym_idx in range(dysymtab.nundefsym):
            ...
            symbol_name = self.binary.get_full_string_from_start_address(
                string_file_address, virtual=False
            )
            if not symbol_name:
                logging.error(
                    f"Could not get symbol name at address {string_file_address}"
                )
                continue
    ...
    def _parse_selrefs(self) -> None:
        ...
        selref_pointers, selector_literal_pointers = self.binary.read_pointer_section(
            "__objc_selrefs"
        )
        # sanity check
        ...
        for selref_ptr, selector_literal_ptr in zip(
            selref_pointers, selector_literal_pointers
        ):
            # read selector string literal from selref pointer
            selector_string = self.binary.get_full_string_from_start_address(
                selector_literal_ptr
            )
            if not selector_string:
                continue  # but all selectors should have a name
            wrapped_selref = ObjcSelref(
                selref_ptr, selector_literal_ptr, selector_string
            )
            ...
            self._selref_ptr_to_selector_map[selref_ptr] = ObjcSelector(
                selector_string, wrapped_selref, None
            )

    def selector_for_selref(
        self, selref_addr: VirtualMemoryPointer
        ...
        selref = [
            x
            for x in self._selector_literal_ptr_to_selref_map.values()
            if x.source_address == selref_addr
        ]
        if not len(selref):
            return None
        _selref = selref[0]
    ...
    def selector_for_selector_literal(
        self, literal_addr: VirtualMemoryPointer
    ) -> Optional[ObjcSelector]:
        return self.selector_for_selref(
            self._selector_literal_ptr_to_selref_map[literal_addr].source_address
        )
    ...
    def selref_for_selector_name(
        ...
        selref_list = [
            x
            for x in self._selref_ptr_to_selector_map
    ...
    def objc_class_for_classlist_pointer(
        self, classlist_ptr: VirtualMemoryPointer
    ) -> Optional[ObjcClass]:
    ...
    def _parse_objc_categories(self) -> List[ObjcCategory]:
        DebugUtil.log(
            self,
            "Cross referencing __objc_catlist, __objc_category, and __objc_data entries...",
        )
        parsed_categories = []
    ...
    def _parse_global_protocol_info(self) -> List[ObjcProtocol]:
        ...
        DebugUtil.log(
            self,
            "Cross referencing __objc_protolist, __objc_protocol, and __objc_data entries...",
        )
        protocol_pointers = self._get_protolist_pointers()
        return self._parse_protocol_ptr_list(protocol_pointers)

    def read_ivars_from_ivarlist_ptr(
        self, ivarlist_ptr: VirtualMemoryPointer
    ) -> List[ObjcIvar]:
        """Given the virtual address of an ivar list, return a List of each encoded ObjcIvar
        """
        ivarlist = self.binary.read_struct(
            ivarlist_ptr, ObjcIvarListStruct, virtual=True
        )
        ivars: List[ObjcIvar] = []
        ...
        for _ in range(ivarlist.count):
            ivar_struct = self.binary.read_struct(
                ivar_struct_ptr, ObjcIvarStruct, virtual=True
            )

            ivar_name = self.binary.get_full_string_from_start_address(ivar_struct.name)
            class_name = self.binary.get_full_string_from_start_address(
                ivar_struct.type
            )
            field_offset = self.binary.read_word(
                ivar_struct.offset_ptr, word_type=c_uint32
            )
            ...
            if not ivar_name:
                raise ValueError(
                    f"Failed to read ivar data for ivar entry @ {hex(ivar_struct_ptr)}"
                )

            ivar = ObjcIvar(
                ivar_name, class_name, field_offset, field_offset_addr
            )  # type: ignore
            ivars.append(ivar)
    ...
    def read_selectors_from_methlist_ptr(
        self, methlist_ptr: VirtualMemoryPointer
    ) -> List[ObjcSelector]:
        """Given the virtual address of a method list, return a List of ObjcSelectors encapsulating each method
        """
        methlist = self.binary.read_struct(
            methlist_ptr, ObjcMethodListStruct, virtual=True
        )
        selectors: List[ObjcSelector] = []
        ...
        for i in range(methlist.methcount):
            method_ent = self.binary.read_struct(
                method_entry_off, ObjcMethodStruct, virtual=True
            )
            # byte-align IMP
            ...
            selector = ObjcSelector(
                symbol_name, selref, VirtualMemoryPointer(method_ent.implementation)
            )
    ...
    def _parse_objc_protocol_entry(
        self, objc_protocol_struct: ObjcProtocolRawStruct
    ) -> ObjcProtocol:
        symbol_name = self.binary.get_full_string_from_start_address(
            objc_protocol_struct.name
        )
        if not symbol_name:
            raise ValueError(
                f"Could not get symbol name for {objc_protocol_struct.name}"
            )

        selectors: List[ObjcSelector] = []
        if objc_protocol_struct.required_instance_methods:
            selectors += self.read_selectors_from_methlist_ptr(
                objc_protocol_struct.required_instance_methods
            )
        if objc_protocol_struct.required_class_methods:
            selectors += self.read_selectors_from_methlist_ptr(
                objc_protocol_struct.required_class_methods
            )
        if objc_protocol_struct.optional_instance_methods:
            selectors += self.read_selectors_from_methlist_ptr(
                objc_protocol_struct.optional_instance_methods
            )
        if objc_protocol_struct.optional_class_methods:
            selectors += self.read_selectors_from_methlist_ptr(
                objc_protocol_struct.optional_class_methods
            )

        return ObjcProtocol(objc_protocol_struct, symbol_name, selectors)

    def _parse_objc_category_entry(
        self, objc_category_struct: ObjcCategoryRawStruct
    ) -> ObjcCategory:
        # TODO(PT): Add in the methods of the base class to the ObjcCategory
        symbol_name = self.binary.get_full_string_from_start_address(
            objc_category_struct.name
        )
        if not symbol_name:
            raise ValueError(
                f"Could not get symbol name for {objc_category_struct.name}"
            )
    ...
    def _parse_objc_data_entry(
        ...
        if objc_data_struct.base_methods:
            selectors += self.read_selectors_from_methlist_ptr(
                objc_data_struct.base_methods
            )
        # if the class doesn't conform to any protocols, base_protocols will be null
        if objc_data_struct.base_protocols:
            protocol_pointer_list = self._protolist_ptr_to_protocol_ptr_list(
                objc_data_struct.base_protocols
            )
        ...
        return ObjcClass(
            ...
            ivars,
            protocols,
            objc_class_struct.superclass,
        )

    def _protolist_ptr_to_protocol_ptr_list(
        self, protolist_ptr: VirtualMemoryPointer
    ) -> List[VirtualMemoryPointer]:
        """Accepts the virtual address of an ObjcProtocolListStruct, and returns List of protocol pointers it refers to.
        """
        protolist = self.binary.read_struct(
            protolist_ptr, ObjcProtocolListStruct, virtual=True
        )
    ...
    def _parse_protocol_ptr_list(
        self, protocol_ptrs: List[VirtualMemoryPointer]
    ) -> List[ObjcProtocol]:
        protocols = []
    ...
    def _get_objc_data_from_objc_class(
        self, objc_class: ObjcClassRawStruct
    ) -> Optional[ObjcDataRawStruct]:
        ...
        """
        data_entry = self.binary.read_struct(
            objc_class.data, ObjcDataRawStruct, virtual=True
        )
