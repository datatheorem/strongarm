...
from strongarm.objc.objc_analyzer import ObjcFunctionAnalyzer, ObjcMethodInfo, ObjcUnconditionalBranchInstruction
...
class StackStorage(VariableStorage):
    def __init__(self, offset: int) -> None:
        self.offset = offset
        super().__init__(self.get_label(offset))

    @staticmethod
    def get_label(offset: int):
        return f'sp+{hex(offset)}'

    def __repr__(self):
        return f'[{self.get_label(self.offset)}]'
...
class Execution:
    ...
    def __init__(self, function_analyzer: ObjcFunctionAnalyzer) -> None:
        ...
        self.set_imm(self.storage_from_stack_offset(0), 0x100000000)

        # Set up the callee-saved registers (x19-x28)
    ...
    def storage_from_stack_offset(self, offset: int) -> VariableStorage:
        # label = StackStorage.get_label(offset)
        if offset == 0:
            label = f'sp+0x0'
        else:
            stack_addr = self.stack_pointer.contents.get_value(self) - offset
            label = f'{hex(stack_addr)}'

        if label not in self._variables:
            self._variables[label] = StackStorage(offset)
        storage = self._variables[label]
    ...
    @property
    def stack_pointer(self) -> VariableStorage:
        return self.storage_from_stack_offset(0)
...
class FunctionInterpreter:
    def __init__(self,
                 ...
                 target_instruction: ObjcUnconditionalBranchInstruction) -> None:
        self.function_analyzer = function_analyzer
    ...
    def _find_function_prologue_end(self) -> InstructionIndex:
        ...
        for idx, instr in enumerate(function_analyzer.instructions):
            ...
            try:
                ...
                if src != self.execution.stack_pointer:
                    continue
    ...
    def _add(self, instr: CsInsn) -> None:
        ...
        assert source == self.execution.stack_pointer

        stack_offset_op = instr.operands[2]
        assert stack_offset_op.type == ARM64_OP_IMM

        stack_offset = stack_offset_op.value.imm
        # PT: now that we actually model the stack pointer, we might need to change how stack space is represented?
        # The label "sp+0x10" changes meaning once the stack pointer is modified
        stack_loc = self.execution.storage_from_stack_offset(stack_offset)

        self.execution.set_contents(dest, StorageLoad(stack_loc))
    ...
    def _sub(self, instr: CsInsn) -> None:
        ...
        assert offset_op.type == ARM64_OP_IMM
        stack_offset = offset_op.value.imm

        dest_val = source_val - stack_offset
    ...
    def _interpret_function_until_target(self) -> None:
        prologue_end_idx = self._find_function_prologue_end()
        # Can't skip function prologue anymore as we needed to model instances being stored in local variables
        for idx, instr in enumerate(self.function_analyzer.instructions):
            print(f'--------- {self.function_analyzer.format_instruction(instr)}')

            if instr.address == 0x0000000100006530:
                print(f'found it')
                pass
