...
def get_source_library_of_imported_symbol(
    analyzer: MachoAnalyzer, symbol_name: str
) -> str:
    ...
    bound_sym = [
        x for x in analyzer.dyld_bound_symbols.values() if x.name == symbol_name
    ][0]
...
def print_binary_symbols(binary: MachoBinary, verbose: bool = True) -> None:
    ...
    for sym in analyzer.imported_symbols:
        ...
        if verbose:
            segment = "(undefined)"
            source_library_info = (
                f"(from {get_source_library_of_imported_symbol(analyzer, sym)})"
            )
